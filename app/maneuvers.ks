...
function create_exec_next_node{
    function def_cfg{
        local cfg is LEXICON().
        set cfg:auto_warp to FALSE.
        set cfg:remove_node to FALSE.
        RETURN cfg.
    }
    function app_run{
        PARAMETER app.
        ...
        local warp_enabled to app:cfg:auto_warp.
        on chars_read_num {
        ...
        ON (warp_enabled <> warp_trigger_enabled) {
            IF NOT warp_trigger_enabled{
                RETURN false.
            }
            if warp_enabled {
                // 10 - seconds before node burn start
        ...
        stop_reading_input().
        IF app:cfg:remove_node {
            REMOVE nd.
            wait 0.
        }
        return TRUE.
    }
    local app is  create_app("EXEC_MANEUVER", app_run@, def_cfg()).
    RETURN app.
}