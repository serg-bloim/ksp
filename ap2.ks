...
local ap_ui_main_debug1 is ui_reg_lbl(ap_ui_main, 12, 11, 20).
local ap_ui_main_debug2 is ui_reg_lbl(ap_ui_main, 12, 12, 20).
local ap_ui_main_debug3 is ui_reg_lbl(ap_ui_main, 12, 13, 20).
local ap_ui_main_debug4 is ui_reg_lbl(ap_ui_main, 12, 14, 20).
local ap_ui_main_debug5 is ui_reg_lbl(ap_ui_main, 12, 15, 20).
local ap_ui_main_debug6 is ui_reg_lbl(ap_ui_main, 12, 16, 20).
local ap_ui_main_debug7 is ui_reg_lbl(ap_ui_main, 12, 17, 20).
local ap_ui_main_debug8 is ui_reg_lbl(ap_ui_main, 12, 18, 20).
...
local prev_vvel is SHIP:VERTICALSPEED.
local prev_vacc is 0.
local prev_timepoint is TIME:SECONDS-1.
...
until 0 {
    ...
    if wp = 0 or not RCS {
        ...
        if not old_rcs {
            SAS OFF.
            set locked_steering to FACING.
            LOCK STEERING to locked_steering.
            show_main_ui.
        }
        ...
        set vvel_current to SHIP:VERTICALSPEED.
        set time_delta to TIME:SECONDS - prev_timepoint.
        set vacc to (vvel_current - prev_vvel) / time_delta.
        set vacc_change to vacc - prev_vacc.
        set mag_delta to target_alt - SHIP:ALTITUDE.
        set vel_target to mag_delta / 10.
        set pitch to velPID:UPDATE(TIME:SECONDS, SHIP:VERTICALSPEED).
        set velPID:SETPOINT to vel_target.

        set vel_delta to vel_target - vvel_current.
        set acc_target to vel_delta / 10.
        set acc_delta to acc_target - vacc.
        set vacc_change_target to acc_delta / 10.
        set vacc_change_delta to vacc_change_target - vacc_change.
        set prev_timepoint to TIME:SECONDS.
        set prev_vvel to vvel_current.
        set prev_vacc to vacc.
        set fine_tune_alt to fine_tune_alt + vacc_change_delta.

        set my_pos_alt to ship:GEOPOSITION:ALTITUDEPOSITION(SHIP:ALTITUDE+fine_tune_alt).
        ...
        ui_prnt_lbl(ap_ui_main_debug1, mag_delta).
        ui_prnt_lbl(ap_ui_main_debug2, vel_target).
        ui_prnt_lbl(ap_ui_main_debug3, vel_delta).
        ui_prnt_lbl(ap_ui_main_debug4, acc_target).
        ui_prnt_lbl(ap_ui_main_debug5, acc_delta).
        ui_prnt_lbl(ap_ui_main_debug6, pitch).
        ui_prnt_lbl(ap_ui_main_alt_tune, pitch).
        ...
        if ABS(azimuth) < 10 {
            ...
            set locked_steering to LOOKDIRUP(direct_3d, SHIP:UP:VECTOR).
        }else{
        ...
        //show_rot(steering_dir * R(pitch,0,0)).
            set locked_steering to steering_dir * R(pitch,0,0).
        }
